...
struct blocking_t
{
  ...
  {
    return clmdep_asio::query(ex, possibly_t());
  }
...
#endif // !defined(__clang__)
  {
    return clmdep_asio::query(ex, always_t());
  }
...
#endif // !defined(__clang__)
  {
    return clmdep_asio::query(ex, never_t());
  }
...
public:
  ...
  {
    return clmdep_asio::query(executor_, p);
  }
  ...
  {
    return clmdep_asio::require(executor_, possibly_t<I>());
  }
  ...
  {
    return clmdep_asio::require(executor_, never_t<I>());
  }
    ...
    noexcept(is_nothrow_require<const Executor&, Property>::value)
  {
    return adapter<decay_t<require_result_t<const Executor&, Property>>>(
        0, clmdep_asio::require(executor_, p));
  }
    ...
    noexcept(is_nothrow_prefer<const Executor&, Property>::value)
  {
    return adapter<decay_t<prefer_result_t<const Executor&, Property>>>(
        0, clmdep_asio::prefer(executor_, p));
  }
