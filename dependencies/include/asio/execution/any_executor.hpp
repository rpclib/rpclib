...
public:
   ...
   * clmdep_asio::require and clmdep_asio::prefer customisation points.
   *
   * For example:
   * @code execution::any_executor<execution::blocking_t::possibly_t> ex = ...;
   * auto ex2 = clmdep_asio::requre(ex, execution::blocking.possibly); @endcode
   */
  ...
  /**
   * Do not call this function directly. It is intended for use with the
   * clmdep_asio::prefer customisation point.
   *
   * For example:
   * @code execution::any_executor<execution::blocking_t::possibly_t> ex = ...;
   * auto ex2 = clmdep_asio::prefer(ex, execution::blocking.possibly); @endcode
   */
  template <typename Property>
   ...
   * size_t n = clmdep_asio::query(ex, execution::occupancy); @endcode
   */
  template <typename Property>
  ...
  /**
   * Throws clmdep_asio::bad_executor if the polymorphic wrapper has no target.
   */
...
public:
  ...
  {
    if (impl_)
      clmdep_asio::detail::ref_count_up(impl_->ref_count_);
  }
  ...
  {
    if (impl_)
      if (clmdep_asio::detail::ref_count_down(impl_->ref_count_))
        delete impl_;
  }
...
private:
  ...
  struct impl_base
  {
    ...
    virtual ~impl_base() {}
    virtual void* get() = 0;
    clmdep_asio::detail::atomic_count ref_count_;
  };
...
public:
  ...
  void execute(F&& f) const
  {
    if (target_)
    {
      ...
      {
        clmdep_asio::detail::non_const_lvalue<F> f2(f);
...
protected:
  ...
  typedef clmdep_asio::detail::executor_function function;
  typedef clmdep_asio::detail::executor_function_view function_view;
...
# pragma warning (disable:4702)
  ...
  static void query_fn_non_void(void*, const void* ex, const void* prop,
      enable_if_t<
        clmdep_asio::can_query<const Ex&, const Prop&>::value
          && is_same<typename Prop::polymorphic_query_result_type, void>::value
      >*)
  {
    clmdep_asio::query(*static_cast<const Ex*>(ex),
  ...
  static Poly require_fn_impl(const void* ex, const void* prop,
      ...
      >*)
  {
    return clmdep_asio::require(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
  ...
  static Poly prefer_fn_impl(const void* ex, const void* prop,
      ...
      >*)
  {
    return clmdep_asio::prefer(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
...
private:
  ...
  {
    return clmdep_asio::query(ex, execution::blocking);
  }
...
//  template <typename...> friend class any_executor;
  ...
  typedef aligned_storage<
      sizeof(clmdep_asio::detail::shared_ptr<void>) + sizeof(void*),
      alignment_of<clmdep_asio::detail::shared_ptr<void>>::value
    >::type object_type;
...
public:
      ...
      >* = 0) const
  {
    ...
    prop_fns_[found::index].query(&result, object_fns_->target(*this),
        &static_cast<const typename found::type&>(p));
    return *clmdep_asio::detail::scoped_ptr<
      typename found::query_result_type>(result);
  }
