...
public:
  ...
  enable_if_t<
    !is_same<decay_t<Arg>, clmdep_asio::error_code>::value
  >
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(clmdep_asio::error_code, Args...) noexcept>
{
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(const clmdep_asio::error_code&, Args...) noexcept>
{
  typedef R type(Args...) & noexcept;
};
      ...
      typename detail::redirect_error_signature<Signature>::type>
{
  ...
  struct init_wrapper
  {
    explicit init_wrapper(clmdep_asio::error_code& ec)
      : ec_(ec)
    {
    }
