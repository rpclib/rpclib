...
public:
  ...
  co_spawn_work_guard(const Executor& ex)
    : executor_(clmdep_asio::prefer(ex, execution::outstanding_work.tracked))
  {
  }
...
struct co_spawn_state
{
  ...
  co_spawn_state(H&& h, const Executor& ex, F&& f)
    : handler(std::forward<H>(h)),
      spawn_work(ex),
      handler_work(clmdep_asio::get_associated_executor(handler, ex)),
...
struct co_spawn_dispatch
{
  ...
  auto operator()(CompletionToken&& token) const
    -> decltype(clmdep_asio::dispatch(std::forward<CompletionToken>(token)))
  {
    return clmdep_asio::dispatch(std::forward<CompletionToken>(token));
  }
};
...
struct co_spawn_post
{
  template <typename CompletionToken>
  auto operator()(CompletionToken&& token) const
    -> decltype(clmdep_asio::post(std::forward<CompletionToken>(token)))
  {
    return clmdep_asio::post(std::forward<CompletionToken>(token));
  }
};
...
public:
  ...
  void operator()(cancellation_type_t type)
  {
    ...
    clmdep_asio::dispatch(ex_, [sig, type]{ sig->emit(type); });
  }
...
public:
  ...
  void operator()(Handler&& handler, F&& f) const
  {
    ...
    auto slot = clmdep_asio::get_associated_cancellation_slot(handler);
