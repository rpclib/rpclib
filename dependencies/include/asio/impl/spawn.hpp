  ...
  void suspend_with(void (*fn)(void*), void* arg)
  {
    if (throw_if_cancelled_)
      if (!!cancellation_state_.cancelled())
        throw_error(clmdep_asio::error::operation_aborted, "yield");
...
template <typename Executor, typename R>
class spawn_handler<Executor, R(clmdep_asio::error_code)>
  : public spawn_handler_base<Executor>
{
public:
  typedef void return_type;
...
template <typename Executor, typename R, typename T>
class spawn_handler<Executor, R(clmdep_asio::error_code, T)>
  : public spawn_handler_base<Executor>
{
public:
  typedef T return_type;
...
template <typename Executor, typename R, typename... Ts>
class spawn_handler<Executor, R(clmdep_asio::error_code, Ts...)>
  : public spawn_handler_base<Executor>
{
public:
  typedef std::tuple<Ts...> return_type;
    ...
    const boost::coroutines::attributes& attributes)
{
  ...
  clmdep_asio::spawn(ex, static_cast<Function&&>(function), attributes);
}
...
template <typename Function>
inline void spawn(const clmdep_asio::io_context::strand& s, Function&& function,
    const boost::coroutines::attributes& attributes)
{
  clmdep_asio::spawn(clmdep_asio::bind_executor(
