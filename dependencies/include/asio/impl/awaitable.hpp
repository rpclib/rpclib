...
public:
  ...
  void* operator new(std::size_t size)
  {
    return clmdep_asio::detail::thread_info_base::allocate(
        ...
        clmdep_asio::detail::thread_context::top_of_thread_call_stack(),
        size);
  }
...
private:
  ...
  clmdep_asio::cancellation_slot parent_cancellation_slot_;
  clmdep_asio::cancellation_state cancellation_state_;
  bool has_executor_;
...
template <typename R, typename Executor>
class awaitable_async_op_handler<R(clmdep_asio::error_code), Executor>
  : public awaitable_thread<Executor>
{
public:
  typedef clmdep_asio::error_code* result_type;
...
template <typename R, typename T, typename Executor>
class awaitable_async_op_handler<R(clmdep_asio::error_code, T), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
...
template <typename R, typename... Ts, typename Executor>
class awaitable_async_op_handler<R(clmdep_asio::error_code, Ts...), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  template <typename... Args>
  void operator()(clmdep_asio::error_code ec, Args&&... args)
  {
    result_.ec_ = &ec;
