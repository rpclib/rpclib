...
template <>
struct coro_error<clmdep_asio::error_code>
{
  static clmdep_asio::error_code invalid()
  {
    return clmdep_asio::error::fault;
  }

  static clmdep_asio::error_code cancelled()
  {
    return clmdep_asio::error::operation_aborted;
  }

  static clmdep_asio::error_code interrupted()
  {
    return clmdep_asio::error::interrupted;
  }

  static clmdep_asio::error_code done()
  {
    return clmdep_asio::error::broken_pipe;
  }
};
...
struct coro_error<std::exception_ptr>
{
  static std::exception_ptr invalid()
  {
    return std::make_exception_ptr(
        clmdep_asio::system_error(
          coro_error<clmdep_asio::error_code>::invalid()));
  }
  ...
  static std::exception_ptr cancelled()
  {
    return std::make_exception_ptr(
        clmdep_asio::system_error(
          coro_error<clmdep_asio::error_code>::cancelled()));
  }
  ...
  static std::exception_ptr interrupted()
  {
    return std::make_exception_ptr(
        clmdep_asio::system_error(
          coro_error<clmdep_asio::error_code>::interrupted()));
  }
  ...
  static std::exception_ptr done()
  {
    return std::make_exception_ptr(
        clmdep_asio::system_error(
          coro_error<clmdep_asio::error_code>::done()));
  }
};
      ...
      -> coroutine_handle<>
    {
      ...
      if constexpr (!coro_promise<Y, R, E, A>::is_noexcept)
      {
            ...
            && hp.cancel->throw_if_cancelled_)
        {
          clmdep_asio::detail::throw_error(
              clmdep_asio::error::operation_aborted, "coro-cancelled");
        }
      }
      ...
      else
      {
        coro.coro_->awaited_from =
          dispatch_coroutine(
              clmdep_asio::prefer(hp.get_executor(),
        ...
        struct cancel_handler
        {
          ...
          {
            clmdep_asio::dispatch(e, [ct, st = st]() mutable
            {
              auto & [sig, state] = *st;
              sig.emit(ct);
            });
      ...
      typename coro_traits<Yield, Return, Executor>::return_type>
{
  ...
  using cancellation_slot_type = clmdep_asio::cancellation_slot;
  ...
  auto await_transform(this_coro::cancellation_state_t) const
  {
    struct exec_helper
    {
      const clmdep_asio::cancellation_state& value;
      ...
      clmdep_asio::cancellation_state await_resume() const noexcept
      {
        return value;
      }
    };
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume
{
  ...
  typedef clmdep_asio::cancellation_slot cancellation_slot_type;
      ...
      std::true_type /* result is void */)
  {
    ...
    {
      if (!the_coro)
        return clmdep_asio::post(exec,
            clmdep_asio::append(std::move(h),
  ...
  {
    const auto exec = clmdep_asio::prefer(
    ...
    clmdep_asio::dispatch(get_executor(),
