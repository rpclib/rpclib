...
struct parallel_group_completion_handler
{
  ...
  parallel_group_completion_handler(Handler&& h)
    : handler_(std::move(h)),
      executor_(
          clmdep_asio::prefer(
            clmdep_asio::get_associated_executor(handler_),
  ...
  void operator()()
  {
    this->invoke(clmdep_asio::detail::make_index_sequence<sizeof...(Ops)>());
  }

  template <std::size_t... I>
  void invoke(clmdep_asio::detail::index_sequence<I...>)
  {
  ...
  void invoke(std::tuple<Args...>&& args)
  {
    this->invoke(std::move(args),
        clmdep_asio::detail::index_sequence_for<Args...>());
  }

  template <typename... Args, std::size_t... I>
  void invoke(std::tuple<Args...>&& args,
      clmdep_asio::detail::index_sequence<I...>)
  {
...
struct parallel_group_state
{
  ...
  clmdep_asio::cancellation_signal cancellation_signals_[sizeof...(Ops)];
...
struct parallel_group_op_handler
{
  ...
  void operator()(Args... args)
  {
    ...
    if (--state_->outstanding_ == 0)
      clmdep_asio::dispatch(std::move(state_->handler_));
  }
  ...
  parallel_group_op_handler<I, Condition, Handler, Ops...>
{
  ...
  struct cancel_proxy
  {
    ...
    void operator()(cancellation_type_t type)
    {
      ...
      {
        clmdep_asio::cancellation_signal* sig = &signal_;
        clmdep_asio::dispatch(executor_,
            [state, sig, type]{ sig->emit(type); });
      }
    }

    std::weak_ptr<parallel_group_state<Condition, Handler, Ops...>> state_;
    clmdep_asio::cancellation_signal signal_;
    executor_type executor_;
  };
...
struct parallel_group_op_launcher
{
      ...
      Condition, Handler, Ops...>>& state)
  {
    ...
    ex_type ex = clmdep_asio::get_associated_executor(op);
    std::move(op)(
    ...
    std::tuple<Ops...>& ops, clmdep_asio::detail::index_sequence<I...>)
{
  ...
  std::shared_ptr<state_type> state = std::allocate_shared<state_type>(
      clmdep_asio::detail::recycling_allocator<state_type,
        clmdep_asio::detail::thread_info_base::parallel_group_tag>(),
...
struct ranged_parallel_group_completion_handler
{
  ...
  {
    this->invoke(
        clmdep_asio::detail::make_index_sequence<
...
struct ranged_parallel_group_state
{
    ...
    : cancellations_requested_(size),
      ...
      cancellation_signals_(
          ASIO_REBIND_ALLOC(Allocator,
            clmdep_asio::cancellation_signal)(allocator)),
      cancellation_condition_(std::move(c)),
  ...
  // The cancellation signals for each operation in the group.
  std::deque<clmdep_asio::cancellation_signal,
    ASIO_REBIND_ALLOC(Allocator, clmdep_asio::cancellation_signal)>
      cancellation_signals_;
