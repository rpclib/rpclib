...
template<typename Signature = void(),
    typename Executor = clmdep_asio::any_io_executor,
...
struct promise_impl<void(Ts...), Executor, Allocator>
{
  ...
  void apply_impl(Func f, clmdep_asio::detail::index_sequence<Idx...>)
  {
  ...
  void apply(Func f)
  {
    apply_impl(std::forward<Func>(f),
        clmdep_asio::detail::make_index_sequence<sizeof...(Ts)>{});
  }
  ...
  struct completion_impl final : completion_base
  {
    ...
    void invoke(Ts&&... ts)
    {
      ...
      using alloc_t = typename std::allocator_traits<
        typename clmdep_asio::decay<Alloc>::type>::template
  ...
  typename clmdep_asio::aligned_storage<sizeof(void*) * 4,
  ...
  void set_completion(Alloc&& alloc, Handler&& handler)
  {
    ...
    using impl_t = completion_impl<
      typename clmdep_asio::decay<Alloc>::type, Handler>;
    using alloc_t = typename std::allocator_traits<
      typename clmdep_asio::decay<Alloc>::type>::template rebind_alloc<impl_t>;
  ...
  void complete_with_result_impl(clmdep_asio::detail::index_sequence<Idx...>)
  {
    auto& result_type = *reinterpret_cast<promise_impl::result_type*>(&result);
  ...
  template<typename... T_>
  void cancel_impl_(clmdep_asio::error_code*, T_*...)
  {
    complete(clmdep_asio::error::operation_aborted, T_{}...);
  }
