...
using clmdep_asio::detail::composed_io_executors;
using clmdep_asio::detail::composed_work;
using clmdep_asio::detail::composed_work_guard;
using clmdep_asio::detail::get_composed_io_executor;
using clmdep_asio::detail::make_composed_io_executors;
using clmdep_asio::detail::recycling_allocator;
using clmdep_asio::detail::throw_error;
...
struct co_composed_state_default_cancellation_on_suspend_impl<Executors,
    Handler, Return, R(clmdep_asio::error_code, Args...), Signatures...>
{
  using promise_type = co_composed_promise<Executors, Handler, Return>;
  using return_type = std::tuple<clmdep_asio::error_code, Args...>;
  ...
  static constexpr void (*fn())(void*)
  {
    if constexpr ((is_constructible<Args>::value && ...))
    {
      return [](void* p)
      {
        ...
        std::move(handler)(
            clmdep_asio::error_code(clmdep_asio::error::operation_aborted),
            Args{}...);
      };
    }
    else
      ...
      return [](void* p)
      {
        ...
        std::move(handler)(
            std::make_exception_ptr(
              clmdep_asio::system_error(
                clmdep_asio::error::operation_aborted, "co_await")),
            Args{}...);
      };
    }
    else
...
private:
  ...
  void check_for_cancellation_on_transform()
  {
    if (throw_if_cancelled_ && !!cancelled())
      throw_error(clmdep_asio::error::operation_aborted, "co_await");
  }
...
class co_composed_handler<Executors, Handler,
    Return, R(clmdep_asio::error_code, Args...)>
  : public co_composed_handler_base<Executors, Handler, Return>
{
public:
  using co_composed_handler_base<Executors,
  ...
  using result_type = std::tuple<clmdep_asio::error_code, args_type>;

  template <typename... T>
  void operator()(const clmdep_asio::error_code& ec, T&&... args)
  {
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
# if defined(ASIO_HAS_SOURCE_LOCATION)
      , clmdep_asio::detail::source_location location
        = clmdep_asio::detail::source_location::current()
# endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
    )
  {
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
# if defined(ASIO_HAS_SOURCE_LOCATION)
          , const clmdep_asio::detail::source_location& location
# endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
        )
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
# if defined(ASIO_HAS_SOURCE_LOCATION)
      clmdep_asio::detail::source_location location_;
# endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
    };
...
struct coroutine_traits<void, C&,
    clmdep_asio::experimental::detail::co_composed_state<
      Executors, Handler, Return>,
    Args...>
{
  using promise_type =
    clmdep_asio::experimental::detail::co_composed_promise<
      Executors, Handler, Return>;
};
