...
auto coro_interpret_result(
    std::tuple<clmdep_asio::error_code, Args...>&& args)
{
  if (std::get<0>(args))
    clmdep_asio::detail::throw_exception(
        clmdep_asio::system_error(std::get<0>(args)));
...
inline auto coro_interpret_result(
    std::tuple<clmdep_asio::error_code>&& args)
{
  if (std::get<0>(args))
    clmdep_asio::detail::throw_exception(
        clmdep_asio::system_error(std::get<0>(args)));
...
template <typename Arg>
auto coro_interpret_result(std::tuple<clmdep_asio::error_code, Arg>&& args)
{
  if (std::get<0>(args))
    clmdep_asio::detail::throw_exception(
        clmdep_asio::system_error(std::get<0>(args)));
