...
struct partial_promise : partial_promise_base<Allocator>
{
  ...
  {
    return clmdep_asio::detail::suspend_always{};
  }
  ...
  auto final_suspend() noexcept
  {
    struct awaitable_t
    {
      ...
      auto await_suspend(clmdep_asio::detail::coroutine_handle<>) noexcept
      {
...
struct coroutine_traits<
    clmdep_asio::experimental::detail::partial_coro,
    Executor, Completion, Args...>
{
  using promise_type =
    clmdep_asio::experimental::detail::partial_promise<
      clmdep_asio::associated_allocator_t<Completion>>;
};
...
{
  post(exec, clmdep_asio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro post_coroutine(Context& ctx,
    CompletionToken token, Args&&... args) noexcept
{
  post(ctx, clmdep_asio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Executor exec,
    CompletionToken token, Args&&... args) noexcept
{
  dispatch(exec, clmdep_asio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Context& ctx,
    CompletionToken token, Args &&... args) noexcept
{
  dispatch(ctx, clmdep_asio::append(std::move(token), std::move(args)...));
  co_return;
}
