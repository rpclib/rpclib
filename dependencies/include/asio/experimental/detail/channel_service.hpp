...
class channel_service
  : public clmdep_asio::detail::execution_context_service_base<
...
public:
  ...
  struct base_implementation_type
  {
    ...
    clmdep_asio::detail::op_queue<channel_operation> waiters_;
...
struct channel_service<Mutex>::implementation_type<
    Traits, R(clmdep_asio::error_code)>
  : channel_service::base_implementation_type
{
  // The traits type associated with the channel.
  typedef typename Traits::template rebind<R(clmdep_asio::error_code)>::other
    traits_type;
  ...
  typedef conditional_t<
      ...
      conditional_t<
        has_signature<
          ...
          R(clmdep_asio::error_code)
        >::value,
        channel_payload<R(clmdep_asio::error_code)>,
        channel_payload<
          R(clmdep_asio::error_code),
          typename traits_type::receive_closed_signature
        >
  ...
  void buffer_push(payload_type payload)
  {
    ...
    {
      clmdep_asio::error_code value{last.value_};
...
private:
  ...
  {
    clmdep_asio::error_code value_;
    std::size_t count_;
  };

  struct value_handler
  {
    clmdep_asio::error_code& target_;

    template <typename... Args>
    void operator()(const clmdep_asio::error_code& value, Args&&...)
    {
      target_ = value;
    }
  };
