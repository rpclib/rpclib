...
class channel_payload<R1(), R2(clmdep_asio::error_code)>
{
public:
  typedef channel_message<R1()> void_message_type;
  typedef channel_message<R2(clmdep_asio::error_code)> error_message_type;

  channel_payload(void_message_type&&)
    : message_(0, clmdep_asio::error_code()),
      empty_(true)
  {
  }
