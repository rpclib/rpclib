 ...
 * @code
 * awaitable<void> read_write_some(clmdep_asio::ip::tcp::socket & sock,
 *     clmdep_asio::mutable_buffer read_buf, clmdep_asio::const_buffer to_write)
 * {
 *   auto p = clmdep_asio::async_read(read_buf, clmdep_asio::use_awaitable);
 *   co_await clmdep_asio::async_write_some(to_write, clmdep_asio::deferred);
 *   co_await p;
 * }
 * @endcode
 */
...
#endif // defined(GENERATING_DOCUMENTATION)
{
  ...
  void cancel(cancellation_type level = cancellation_type::all)
  {
    if (impl_ && !impl_->done)
    {
      clmdep_asio::dispatch(impl_->executor,
...
private:
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(WaitHandler&& handler) const
    {
      ...
      if (self_->done)
      {
        auto exec = clmdep_asio::get_associated_executor(
            handler, self_->get_executor());

        clmdep_asio::post(exec,
            [self = std::move(self_),
