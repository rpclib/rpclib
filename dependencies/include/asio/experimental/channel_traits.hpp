...
template <typename R>
struct channel_traits<R(clmdep_asio::error_code)>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(clmdep_asio::error_code);

  template <typename F>
  static void invoke_receive_cancelled(F f)
  {
    const clmdep_asio::error_code e = error::channel_cancelled;
    static_cast<F&&>(f)(e);
  }

  typedef R receive_closed_signature(clmdep_asio::error_code);

  template <typename F>
  static void invoke_receive_closed(F f)
  {
    const clmdep_asio::error_code e = error::channel_closed;
    static_cast<F&&>(f)(e);
  }
};

template <typename R, typename... Args, typename... Signatures>
struct channel_traits<R(clmdep_asio::error_code, Args...), Signatures...>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(clmdep_asio::error_code, Args...);
  ...
  typedef R receive_closed_signature(clmdep_asio::error_code, Args...);
...
struct channel_traits<R(std::exception_ptr)>
{
  ...
  static void invoke_receive_cancelled(F f)
  {
    const clmdep_asio::error_code e = error::channel_cancelled;
    static_cast<F&&>(f)(
        std::make_exception_ptr(clmdep_asio::system_error(e)));
  }
