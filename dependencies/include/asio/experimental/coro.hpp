...
struct coro
{
  /// The traits of the coroutine. See clmdep_asio::experimental::coro_traits
  /// for details.
  ...
  ~coro()
  {
    if (coro_ != nullptr)
    {
      ...
      if (handle)
        clmdep_asio::dispatch(coro_->get_executor(), destroyer{handle});
    }
  }
...
template<typename T, typename Executor = clmdep_asio::any_io_executor,
    typename Allocator = std::allocator<void>>
