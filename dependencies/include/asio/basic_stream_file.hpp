...
public:
  ...
  uint64_t seek(int64_t offset, file_base::seek_basis whence)
  {
    ...
    clmdep_asio::detail::throw_error(ec, "seek");
    return n;
  }
   ...
   * clmdep_asio::error::eof indicates that the end of the file was reached.
   *
   ...
   * @code
   * file.write_some(clmdep_asio::buffer(data, size));
   * @endcode
   ...
   * @code void(clmdep_asio::error_code, std::size_t) @endcode
   *
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * file.async_write_some(clmdep_asio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * cancellation for the following clmdep_asio::cancellation_type values:
   *
   ...
   * @code
   * file.read_some(clmdep_asio::buffer(data, size));
   * @endcode
   ...
   * To read into a single data buffer use the @ref buffer function as follows:
   * @code
   * file.async_read_some(clmdep_asio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on reading into multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
