    ...
    int op_type, void* cancellation_key)
{
  ...
  bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
      descriptor, ops, cancellation_key, clmdep_asio::error::operation_aborted);
    ...
    const clmdep_asio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
{
  if (owner)
  {
    ...
    clmdep_asio::detail::mutex::scoped_lock lock(reactor->mutex_);
    ...
    clmdep_asio::detail::signal_blocker sb;
    reactor->thread_ =
      new clmdep_asio::detail::thread(thread_function(reactor));
  }
}
#endif // defined(ASIO_HAS_IOCP)
