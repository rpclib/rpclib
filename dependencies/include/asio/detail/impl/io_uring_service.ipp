...
io_uring_service::io_uring_service(clmdep_asio::execution_context& ctx)
    ...
    clmdep_asio::execution_context::fork_event fork_ev)
{
  switch (fork_ev)
  {
  case clmdep_asio::execution_context::fork_prepare:
    {
  ...
  case clmdep_asio::execution_context::fork_parent:
    // Restart the timeout and eventfd operations.
    update_timeout();
    register_with_reactor();
    break;

  case clmdep_asio::execution_context::fork_child:
    {
    ...
    int op_type, io_uring_operation* op)
{
  ...
  {
    clmdep_asio::error_code ec(ENOBUFS,
        clmdep_asio::error::get_system_category());
    clmdep_asio::detail::throw_error(ec, "io_uring_get_sqe");
  }
}
...
void io_uring_service::register_buffers(const ::iovec* v, unsigned n)
{
  ...
  if (result < 0)
  {
    clmdep_asio::error_code ec(-result,
        clmdep_asio::error::get_system_category());
    clmdep_asio::detail::throw_error(ec, "io_uring_register_buffers");
  }
}
...
void io_uring_service::init_ring()
{
  ...
  if (result < 0)
  {
    ...
    clmdep_asio::error_code ec(-result,
        clmdep_asio::error::get_system_category());
    clmdep_asio::detail::throw_error(ec, "io_uring_queue_init");
  }
  ...
  if (event_fd_ < 0)
  {
    clmdep_asio::error_code ec(-result,
        clmdep_asio::error::get_system_category());
    ::io_uring_queue_exit(&ring_);
    clmdep_asio::detail::throw_error(ec, "eventfd");
  }
...
public:
  event_fd_read_op(io_uring_service* s)
    : reactor_op(clmdep_asio::error_code(),
...
operation* io_uring_service::io_queue::perform_io(int result)
{
  ...
  if (result != -ECANCELED || cancel_requested_)
  {
    if (io_uring_operation* op = op_queue_.front())
    {
      if (result < 0)
      {
        op->ec_.assign(-result, clmdep_asio::error::get_system_category());
  ...
  if (!op_queue_.empty())
  {
    ...
    else
    {
      ...
      while (io_uring_operation* op = op_queue_.front())
      {
        op->ec_ = clmdep_asio::error::no_buffer_space;
        op_queue_.pop();
        io_cleanup.ops_.push(op);
      }
    }
