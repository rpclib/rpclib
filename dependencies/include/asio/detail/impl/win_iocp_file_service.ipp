...
clmdep_asio::error_code win_iocp_file_service::open(
  ...
  if (handle != INVALID_HANDLE_VALUE)
  {
    if (disposition == OPEN_ALWAYS && (open_flags & file_base::truncate) != 0)
    {
      if (!::SetEndOfFile(handle))
      {
        ...
        ::CloseHandle(handle);
        ec.assign(last_error, clmdep_asio::error::get_system_category());
        ASIO_ERROR_LOCATION(ec);
        return ec;
      }
    }
...
clmdep_asio::error_code win_iocp_file_service::resize(
    win_iocp_file_service::implementation_type& impl,
    uint64_t n, clmdep_asio::error_code& ec)
{
...
clmdep_asio::error_code win_iocp_file_service::sync_all(
    win_iocp_file_service::implementation_type& impl,
    clmdep_asio::error_code& ec)
{
...
clmdep_asio::error_code win_iocp_file_service::sync_data(
    win_iocp_file_service::implementation_type& impl,
    clmdep_asio::error_code& ec)
{
...
uint64_t win_iocp_file_service::seek(
    ...
    file_base::seek_basis whence, clmdep_asio::error_code& ec)
{
  DWORD method;
  switch (whence)
  {
