...
socket_type sync_accept(socket_type s, state_type state,
    void* addr, std::size_t* addrlen, clmdep_asio::error_code& ec)
{
  // Accept a socket.
  for (;;)
  {
    ...
    std::size_t addrlen, clmdep_asio::error_code& ec)
{
  ...
  if (result != 0 && ec == clmdep_asio::error::try_again)
  {
    if (static_cast<const socket_addr_type*>(addr)->sa_family == AF_UNIX)
      ec = clmdep_asio::error::in_progress;
    else
...
size_t sync_recv1(socket_type s, state_type state, void* data,
    size_t size, int flags, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
signed_size_type recvfrom(socket_type s, buf* bufs, size_t count,
    int flags, void* addr, std::size_t* addrlen, clmdep_asio::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  DWORD recv_buf_count = static_cast<DWORD>(count);
...
int getpeername(socket_type s, void* addr, std::size_t* addrlen,
    bool cached, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
clmdep_asio::error_code getnameinfo(const void* addr,
...
clmdep_asio::error_code sync_getnameinfo(const void* addr,
