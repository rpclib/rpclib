...
public:
	...
	explicit BlockingConcurrentQueue(size_t capacity = 6 * BLOCK_SIZE)
		: inner(capacity), sema(create<LightweightSemaphore>(), &BlockingConcurrentQueue::template destroy<LightweightSemaphore>), canceled_(false)
	{
...
public:
	...
	{
		sema->wait();
		while (!inner.try_dequeue(item) && !canceled_) {
			continue;
		}
	}
    ...
    // added for callme: allows stopping the waiting thread. 
    // external flags are checked in this case.
    void cancel_wait() {
        canceled_ = true;
        sema->signal();
    }
