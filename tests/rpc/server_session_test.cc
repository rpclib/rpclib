...
public:
  server_session_test() : s("127.0.0.1", test_port), c("127.0.0.1", test_port) {
...
TEST(server_session_test_bug153, bug_153_crash_on_client_timeout) {
  ...
  auto client = std::unique_ptr<rpc::client>(
      new rpc::client("localhost", rpc::constants::DEFAULT_PORT));
