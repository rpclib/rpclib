...
public:
  client_test() : s("127.0.0.1", test_port), is_running_(false) {
    ...
    s.bind("close", []() { rpc::this_session().post_exit(); });
    s.async_run();
  }
...
TEST_F(client_test, timeout_right_msg) {
  ...
  try {
    ...
    std::stringstream ss;
    ss << "rpc::timeout: Timeout of " << *client.get_timeout()
...
TEST_F(client_test, cb_test) {
  rpc::client client("127.0.0.1", test_port);
  client.set_state_handler([](rpc::client *client, rpc::connection_state prev,
