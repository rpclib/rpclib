...
public:
  this_server_test()
      : s(test_port), c1("127.0.0.1", test_port), c2("127.0.0.1", test_port) {}
...
TEST_F(this_server_test, stop) {
  ...
  EXPECT_EQ(c1.get_connection_state(), connection_state::disconnected);
  EXPECT_EQ(c2.get_connection_state(), connection_state::disconnected);
}
