...
private:
  ...
  static std::string now() {
    ...
    ss << std::put_time(
              ...
              "%F %T")
       << RPCLIB_FMT::format(".{:03}",
                             round(static_cast<double>(now_t.tv_nsec) / 1.0e6));
    return ss.str();
  }
#endif

  void basic_log(const char *severity,
                 const char *channel,
...
#define LOG_DEBUG(...)                                                        \
  rpc::detail::logger::instance().debug(rpc_channel_name, __FILE__, __func__, \
                                        __LINE__, __VA_ARGS__)

#define LOG_TRACE(...)                                                        \
  rpc::detail::logger::instance().trace(rpc_channel_name, __FILE__, __func__, \
                                        __LINE__, __VA_ARGS__)
